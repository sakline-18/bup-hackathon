...
> **Historical planning document.** It predates the implementation and is partly out of date: the config is `next.config.ts` (not `.mjs`), and the LLM key is `GROQ_API_KEY` (not `LLM_API_KEY`). See [README.md](README.md) for current setup.
